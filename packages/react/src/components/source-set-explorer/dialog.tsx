...
/**
 * The SourceSet explorer's replacement for `window.prompt` / `window.confirm`.
 *
 * Copied from `file-explorer/file-explorer-dialog.tsx` under F-025's "先在本票內複製一份" rule; F-027
 * folds the two back together. The reasoning below is the original's and still applies verbatim.
 *
...
export interface SourceSetDialogApi {
...
export function useSourceSetDialog(locale: Locale): SourceSetDialogApi {
      ...
      <div className={styles.dialog} role="dialog" aria-labelledby={titleId}>
        ...
        <div className={styles.actions}>
          <button type="button" className={styles.cancel} onClick={() => settle(false)}>
            {t(locale, 'sourceSetExplorer.cancel')}
          </button>
          <button
            type="button"
          ...
          >
            {t(locale, 'sourceSetExplorer.confirm')}
          </button>
        </div>
      </div>
    </div>
