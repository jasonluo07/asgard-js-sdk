...
// the composer. The mock behind `docked-run-chrome-demo` streams for ~40s and mutates the strip mid-run,
// which is what makes the difference visible; the `-empty-` variant streams the same thread with no run
// chrome, for the "no strip → no gap" case.
const config = {
...
export function DockedRunChromeRoute(): ReactNode {
  const [withRunChrome, setWithRunChrome] = useState(true);
  const [hideRunChrome, setHideRunChrome] = useState(false);

  const channelId = withRunChrome ? 'docked-run-chrome-demo' : 'docked-run-chrome-empty-demo';

  return (
    <DemoWrapper
      title="Docked run-chrome positioning (BUG-003)"
      description="Task / Subagent 面板改釘在 thread 的 scroll 匡之外、composer 之上的固定區。載入頁面即開始一段約 40 秒的長串流：對話串會遠超出可視範圍並持續自動捲動，用來檢查面板是否穩定停在原位、不隨訊息捲動或串流位移。"
    >
      <div className={styles.legend}>
        ...
        <div className={styles.controls}>
          <button type="button" className={styles.toggle} onClick={(): void => setWithRunChrome(v => !v)}>
            {withRunChrome ? '切到「無 run chrome」（固定區應完全消失）' : '切回「有 run chrome」'}
          </button>
