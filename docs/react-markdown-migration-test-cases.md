...
describe('useMarkdownRenderer interface', () => {
  ...
  it('should respect custom delay parameter', () => {
    ...
    const { rerender } = renderHook(({ text }) => useMarkdownRenderer(text, customDelay), {
...
describe('streaming performance', () => {
  it('should handle rapid text updates efficiently', async () => {
    const { rerender } = renderHook(({ text }) => useMarkdownRenderer(text, 10), { initialProps: { text: '' } });
...
describe('caching behavior', () => {
  ...
  it('should clear cache when text changes completely', () => {
    const { rerender } = renderHook(({ text }) => useMarkdownRenderer(text), {
...
describe('TextTemplate integration', () => {
  ...
  it('should preserve theme integration', () => {
    ...
    const ThemeProvider = ({ children }) => (
      <AsgardThemeContextProvider value={{ botMessage: { color: 'red' } }}>{children}</AsgardThemeContextProvider>
    );
