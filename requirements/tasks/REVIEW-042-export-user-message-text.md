...
- Reviewed commit: `28f7749` + uncommitted working tree (branch `fix/53-export-user-message-text`, pre-commit)
...
| 新增公開元件從 package 進入點導出（`export type` 明確）        | FRONTEND_RULE_COMMON §2.2      | ✅ `text-template/index.ts` → `templates` → `components` → root；`UserMessageTextProps` 為 `export interface` |
...
`apps/react-demo/src/app/routes/composed-bot-text`。
...
| R3  | 自組 `TemplateBox type="user"` + `UserMessageText` 與預設列視覺一致、可放 JSX                                        | Pass   | Demo `/composed-bot-text` @1280×900：Composed / Default 的 user 泡泡 `left 539/538`、`width 257/257`、`background rgb(71,103,235)`、`padding 8px 12px`、`border-radius 8px 0 8px 8px`、`max-width 75%`、`color rgb(255,255,255)`。1px 位移為兩列共有（Default 因保留時間戳而較高、出現 scrollbar），非泡泡本身差異。mention chip（`<span>`）正常渲染於泡泡內。 |
